...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const code = String(courseCode || "").trim().toUpperCase().replace(/[^A-Z0-9]/g, "");
    if (!code) return res.status(400).json({ error: "Course code is required." });

    const profile = await db.collection("users").doc(decoded.uid).get();
    if (!profile.exists || !profile.data().matric) return res.status(400).json({ error: "Valid user profile required." });
    const matric = String(profile.data().matric).trim().toUpperCase();
    const matches = await db.collection("courses").get();
    const course = matches.docs.find((item) => String(item.data().code || "").toUpperCase().replace(/[^A-Z0-9]/g, "") === code);
    ...
    if (!existing.exists) {
      await memberRef.create({ uid: decoded.uid, matric, role: "student", joinedAt: admin.firestore.FieldValue.serverTimestamp() });
    }
