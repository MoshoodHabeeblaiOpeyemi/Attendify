...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // 🔒 FAILED-PIN LOCKOUT CHECK: a 4-digit PIN with zero rate limiting
    // lets an enrolled student enumerate all 10,000 PINs mid-session. Track
    // attempts per (uid, session) and lock out after 5 strikes for the rest
    // of the session. On lockout, auto-create a manual verification request
    // so the student has a recovery path.
    const MAX_PIN_ATTEMPTS = 5;
    const sessionExpiresAt = liveDoc.data().expiresAt;
    const pinAttemptRef = courseRef.collection("pinAttempts").doc(uid);
    const pinAttemptDoc = await pinAttemptRef.get();
    if (pinAttemptDoc.exists) {
      const attemptData = pinAttemptDoc.data();
      if (
        attemptData.sessionExpiresAt === sessionExpiresAt &&
        attemptData.lockedUntil &&
        now < attemptData.lockedUntil
      ) {
        return res.status(423).json({
          error:
            "Too many incorrect PIN attempts. You are locked out for this session. A manual verification request has been sent to your Rep — raise your hand.",
          lockedOut: true,
        });
      }
    }
    const currentPin = String(sessionData.pin || "");
    const previousPin = sessionData.previousPin
      ? String(sessionData.previousPin)
      : null;
    ...
    const pinRotationIntervalMs = (sessionData.pinRotationInterval || 10) * 1000;

    // 🔒 SERVER-SIDE PIN AGE CHECK: if the rep's tab closes, pinRotationTime
    // freezes and the "current" PIN would stay valid forever — anti-relay
    // silently off. Reject the current PIN when it's aged past 2 rotation
    // intervals (e.g. 20s for a 10s rotation). No scheduler needed — dead
    // PINs die even if rotation stalls.
    const pinAge = now - pinRotationTime;
    const isCurrentPinFresh = pinAge < pinRotationIntervalMs * 2;
    const isCurrentPinValid =
      String(pin) === currentPin && isCurrentPinFresh;
    const isPreviousPinValid =
      previousPin &&
      String(pin) === previousPin &&
      pinAge < gracePeriodMs;

    if (!isCurrentPinValid && !isPreviousPinValid) {
      // Distinguish stale-PIN from wrong-PIN so the client can show the right message
      if (String(pin) === currentPin && !isCurrentPinFresh) {
        return res.status(400).json({
          error:
            "This PIN has expired. Please ask your Rep for the current PIN.",
          pinExpired: true,
        });
      }

      // 🔒 FAILED-PIN COUNTER: increment attempts for this (uid, session).
      // After MAX_PIN_ATTEMPTS strikes, lock the student out for the rest
      // of the session and auto-create a manual verification request.
      const prevAttempts = pinAttemptDoc.exists
        ? pinAttemptDoc.data().attempts || 0
        : 0;
      const newAttempts = prevAttempts + 1;
      const remainingAttempts = Math.max(0, MAX_PIN_ATTEMPTS - newAttempts);

      if (newAttempts >= MAX_PIN_ATTEMPTS) {
        // Lock out for the rest of the session
        await pinAttemptRef.set({
          uid,
          sessionExpiresAt,
          attempts: newAttempts,
          lockedUntil: sessionExpiresAt, // locked until session expires
          lastAttemptAt: FieldValue.serverTimestamp(),
        });

        // Auto-create a manual verification request so the student has a
        // recovery path — raise their hand, rep verifies physically.
        try {
          const userDoc = await db.collection("users").doc(uid).get();
          const userName = userDoc.exists
            ? String(userDoc.data().name || "").trim() || matric
            : matric;
          await courseRef
            ...
            .set({
              ...
              matric,
              reason:
                "Auto-generated: locked out after 5 incorrect PIN attempts. Please verify physically.",
              status: "pending",
              sessionExpiresAt,
          ...
          console.warn(
            "Auto manual request creation failed (lockout still enforced):",
            reqErr.message,
          );
        }
      ...
      // Not yet locked out — record the attempt and warn
      await pinAttemptRef.set({
        uid,
        sessionExpiresAt,
        attempts: newAttempts,
      ...
      return res.status(400).json({
        error:
          remainingAttempts <= 2
            ...
            : "Invalid attendance PIN. Please ask your Rep for the current PIN.",
        remainingAttempts,
      });
    }
